...
import {
  MapPin, Bus, Plus, Trash2, FileText,
  ArrowLeft, Check,
} from "lucide-react";
...
const DRAFT_KEY = "busRoute_draft";
...
export default function CreateRoute({ onCancel, onSaved }) {
  const dispatch = useDispatch();

  // ── Redux state ──
  const isCreating  = useSelector(selectBusRouteLoading("createBusRoute"));
  ...
  useEffect(() => {
    try {
      ...
      if (draft.endPoint)   setEndPoint(draft.endPoint);
      if (draft.distance)   setDistance(draft.distance);
      if (draft.buses?.length)  setBuses(draft.buses);
  ...
  // ── Mirror Redux notifications → sonner toasts ──
  useEffect(() => {
  ...
  }, [notification, dispatch, onSaved]);

  const handleSaveDraft = () => {
    try {
      const draft = { name, status, startPoint, endPoint, distance, buses, stops };
      localStorage.setItem(DRAFT_KEY, JSON.stringify(draft));
      toast.success("Draft saved!", {
        description: "Your progress has been saved locally. It will be restored next time you open Create Route.",
      });
    } catch {
      toast.error("Could not save draft. Storage may be unavailable.");
    }
  };

  // ── Bus helpers ──
  const handleAddBus = () =>
    setBuses((prev) => [...prev, { id: `bus-${Date.now()}`, number: "", plate: "" }]);
  ...
  // ── Stop helpers ──
  const handleAddStop    = () =>
    setStops((prev) => [...prev, { id: `stop-${Date.now()}`, name: "" }]);
  ...
  // ── Validate & build payload matching backend createBusRouteService ──
  const buildPayload = () => {
    if (!name.trim())                       return { error: "Route name is required." };
    if (!startPoint.trim())                 return { error: "Start point is required." };
    if (!endPoint.trim())                   return { error: "End point is required." };
    if (!distance || parseFloat(distance) <= 0)
      return { error: "Please enter a valid distance (km)." };

    // Only keep buses with at least a bus number
    const validBuses = buses
      .filter((b) => b.number.trim())
      .map((b) => ({ busNo: b.number.trim(), plateNumber: b.plate.trim() }));
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(createBusRoute(result.payload));
  };

  // ── Derived counts ──
  const totalValidStops = stops.filter((s) => s.name.trim()).length;
  const totalValidBuses = buses.filter((b) => b.number.trim()).length;
    ...
    <div className="flex-1 flex flex-col gap-6 bg-transparent text-slate-800 pb-10 overflow-y-auto pr-2 min-h-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shrink-0 bg-white/40 p-1 rounded-2xl">
        <div>
          <div className="flex items-center gap-2">
            <button
              onClick={onCancel}
              disabled={isCreating}
              className="p-2 -ml-2 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-slate-700 transition-all cursor-pointer disabled:opacity-50"
            >
        ...
        <div className="flex items-center gap-3">
          {/* Draft — saves form state to localStorage */}
          <button
            type="button"
            disabled={isCreating}
            onClick={handleSaveDraft}
            className="flex items-center justify-center gap-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 text-xs font-extrabold px-4 py-2.5 rounded-xl transition-all cursor-pointer shadow-sm disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={isCreating}
            className="flex items-center justify-center gap-2 bg-[#0061FF] hover:bg-blue-700 text-white text-xs font-extrabold px-5 py-2.5 rounded-xl shadow-md shadow-blue-500/10 transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {isCreating ? (
              <>
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <input
                  ...
                  disabled={isCreating}
                  className="block w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 hover:border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all outline-none disabled:opacity-60"
                />
                <span className="text-[11px] text-slate-400 font-medium mt-1.5 block italic">Must be unique within the school.</span>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  disabled={isCreating}
                  className="block w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 hover:border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all outline-none cursor-pointer disabled:opacity-60"
                >
            ...
            <div className="flex flex-col md:flex-row items-end gap-0 relative mb-6">
              ...
              <div className="relative flex-1 flex items-center justify-center shrink-0 pb-[22px]">
                ...
                <button
                  ...
                  disabled={isCreating}
                  className="relative z-10 w-[52px] h-[52px] rounded-full bg-blue-50 border-2 border-blue-200 flex items-center justify-center hover:bg-blue-100 transition-all cursor-pointer disabled:opacity-50"
                  title="Swap Points"
                >
            ...
            <div>
              ...
              <input
                ...
                disabled={isCreating}
                className="block w-full md:w-1/2 px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 hover:border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all outline-none disabled:opacity-60"
              />
              <span className="text-[11px] text-slate-400 font-medium mt-1.5 block italic">Total distance in kilometres.</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm text-left">
          <div className="flex items-start justify-between mb-5">
            ...
            <button
              ...
              disabled={isCreating}
              className="flex items-center gap-1.5 text-sm font-extrabold text-[#0061FF] bg-white hover:bg-blue-50 px-4 py-2.5 rounded-xl border border-blue-300 transition-all cursor-pointer shrink-0 disabled:opacity-50"
            >
              ...
              <div key={bus.id} className="flex items-center gap-4 border border-slate-800/10 rounded-2xl px-5 py-4 bg-white">
                ...
                <div className="flex-1">
                  ...
                  <input
                    ...
                    disabled={isCreating}
                    className="block w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 hover:border-slate-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all outline-none disabled:opacity-60"
                  />
                </div>
                ...
                <button
                  ...
                  disabled={isCreating}
                  className="p-2 text-red-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all cursor-pointer shrink-0 mt-5 disabled:opacity-40"
                  title="Remove Bus"
                >
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm text-left">
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 font-semibold mt-0.5">Add stops in the order the bus will travel.</p>
              </div>
            </div>
            <button
              type="button"
                ...
                <div className="flex-1 flex items-center justify-between border border-slate-200 rounded-2xl px-5 py-3.5 bg-white">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-800">
                      {startPoint ? `${startPoint} (Start Point)` : "Start Point"}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 flex items-center gap-4 border border-slate-200 rounded-2xl px-5 py-3 bg-white">
                    <div className="flex-1">
                      ...
                      <input
                        ...
                        disabled={isCreating}
                        className="block w-full bg-transparent text-sm font-semibold text-slate-800 focus:outline-none disabled:opacity-60"
                      />
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        disabled={isCreating}
                        className="p-2 rounded-lg hover:bg-red-50 text-red-400 hover:text-red-500 transition-all cursor-pointer disabled:opacity-40"
                        title="Delete Stop"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
              ...
              <div className="relative flex items-center gap-4">
                ...
                <div className="flex-1 flex items-center justify-between border border-slate-200 rounded-2xl px-5 py-3.5 bg-white">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-800">
                      {endPoint ? `${endPoint} (End Point)` : "End Point"}
                    </span>
                  </div>
          ...
          <div className="mt-5 flex items-center gap-2 text-sm font-bold text-[#0061FF] bg-blue-50/60 border border-blue-100 px-5 py-3 rounded-xl">
            ...
            <span>Total Stops: {totalValidStops}</span>
          </div>
        </div>

        {/* Sections 4 & 5 — Preview + Transport Fees */}
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm text-left">
            ...
            <div className="flex gap-4">
              <div className="flex-1 space-y-3.5">
                <PreviewRow icon={<Bus className="w-4 h-4 text-slate-400 shrink-0" />} label="Route Name"  value={name || "—"} />
                <PreviewRow icon={<MapPin className="w-4 h-4 text-slate-400 shrink-0" />} label="Total Stops" value={totalValidStops} />
                <PreviewRow icon={<Bus className="w-4 h-4 text-slate-400 shrink-0" />}   label="Total Buses" value={totalValidBuses} />
                <PreviewRow
                  icon={<svg className="w-4 h-4 text-slate-400 shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101" /><path d="M10.172 13.828a4 4 0 015.656 0l4 4a4 4 0 01-5.656 5.656l-1.102-1.101" /></svg>}
                  label="Distance"
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm text-left">
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex gap-3.5 mb-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-semibold leading-relaxed">
                  You can configure stop-wise transport fee structures after saving this route.
                </p>
              </div>
            </div>
            ...
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1-4H9m6 0a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
...
function PreviewRow({ icon, label, value }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-xs font-extrabold text-slate-800 truncate">{value}</span>
    </div>
  );
}